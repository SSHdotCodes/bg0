...
BG0 uses full BiRefNet on capable GPUs and CPU-only browsers. Devices with limited memory or GPU capabilities use BiRefNet-lite. If a model cannot load or run, BG0 retries with a compatible model automatically. Processing time varies by device; the WebAssembly compatibility path is slower than WebGPU.
